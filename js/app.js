class Application {
    ...
    _loadSampleContent() {
        const blocks = [
            ...
            new window.Types.ContentBlock(window.Types.BlockType.PARAGRAPH, {
                text: '欢迎使用交互式排版引擎与分页预览工具！本工具基于最小不齐度（Minimum Raggedness）断行算法，提供专业的页面排版效果。您可以在左侧添加各种内容块，实时在中间预览排版结果，同时在右侧调整排版参数。',
                inlineStyles: []
            }),
            ...
            new window.Types.ContentBlock(window.Types.BlockType.PARAGRAPH, {
                text: '在左侧面板，您可以添加以下类型的内容块：标题（H1-H3，字号不同）、正文段落（支持**加粗**和*斜体*的行内标记）、图片占位（可指定宽高比）、表格和脚注引用。每个内容块都可以通过拖拽调整顺序，或使用操作按钮进行编辑、复制和删除。',
                inlineStyles: [
                    new window.Types.InlineStyle(window.Types.InlineStyleType.BOLD, 56, 58),
                    new window.Types.InlineStyle(window.Types.InlineStyleType.ITALIC, 61, 63)
                ]
            }),
            new window.Types.ContentBlock(window.Types.BlockType.H2, {
                text: '二、排版参数'
            ...
            new window.Types.ContentBlock(window.Types.BlockType.PARAGRAPH, {
                text: '右侧面板提供了丰富的排版参数配置：纸张尺寸（默认A4纵向210×297mm）、上下左右边距（默认各20mm）、正文字号（默认12pt）、行间距（默认1.5倍）、段间距（默认0.5倍行高）以及页眉页脚的显示开关。所有参数修改后都会立即触发重新排版。',
                inlineStyles: []
            }),
            ...
            new window.Types.ContentBlock(window.Types.BlockType.PARAGRAPH, {
                text: '本引擎采用动态规划算法实现最小不齐度断行，通过最小化每行末尾空白量的平方和，产生视觉上更均匀的右边距。中文按字断行，英文按单词断行，超长单词会自动添加连字符。分页规则包含：标题后至少跟随2行正文、防止寡行（段落末行单独出现在下一页）和孤行（段落首行单独留在上一页）、图片整体移动、长表格跨页拆分并重复表头。',
                inlineStyles: []
            }),
            ...
            new window.Types.ContentBlock(window.Types.BlockType.PARAGRAPH, {
                text: '这里插入一些测试文本以演示分页效果。为了展示脚注功能，我们在这里添加一个脚注引用。更多的内容会让分页效果更加明显，包括孤行寡行控制、图片浮动排版等高级特性。通过调整参数，您可以看到排版引擎如何智能地处理各种复杂的排版场景。分页并不是简单的内容截断，而是经过精心计算的优化结果。',
                inlineStyles: [
                    new window.Types.InlineStyle(window.Types.InlineStyleType.FOOTNOTE_REF, 33, 38, {
                        footnoteId: 'fn_001',
                        footnoteText: '这是一个示例脚注，用于说明脚注排版功能。脚注会出现在当页底部，并有分隔线与正文区分。如果脚注内容很长，系统会自动将其续排到下一页。'
                    })
                ]
            }),
            new window.Types.ContentBlock(window.Types.BlockType.H2, {
            ...
            new window.Types.ContentBlock(window.Types.BlockType.PARAGRAPH, {
                text: '中间预览面板支持50%-200%的缩放浏览，当前编辑的内容块会在预览中自动高亮显示并滚动到对应位置。点击右上角的"导出HTML"按钮，可以生成一个自包含的HTML文件，使用CSS @page规则和print media query保持与预览一致的分页效果，打开后可以直接打印出与预览相同的排版结果。',
                inlineStyles: []
            }),
            new window.Types.ContentBlock(window.Types.BlockType.H2, {
                text: '六、脚注说明'
            ...
            new window.Types.ContentBlock(window.Types.BlockType.PARAGRAPH, {
                text: '感谢使用本排版工具！如有任何问题或建议，欢迎反馈。本工具演示了现代排版引擎的核心技术原理，包括专业断行、智能分页、图文混排、脚注处理等功能，希望能对您的工作有所帮助。',
                inlineStyles: []
            })
        ];
