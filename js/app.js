class Application {
    ...
    _loadSampleContent() {
        ...
        const blocks = [
            ...
            p('页边批注功能允许您在正文任意位置添加批注标记，批注内容显示在页面右侧的旁注栏中，通过一条细灰线连接到正文对应位置。这是一个测试段落，用于演示批注功能的显示效果。'),
            new window.Types.ContentBlock(window.Types.BlockType.MARGIN_NOTE, {
                noteText: '这是第一条页边批注，用于说明旁注栏的布局效果。批注内容会显示在页面右侧边距外的旁注栏中。',
                anchoredBlockId: ''
            }),
            p('批注编号是全文连续递增的。每条批注前面带编号，批注的垂直位置尽量对齐正文里对应的锚定标记位置。如果多条批注在同一页里垂直位置太近，后面的批注会自动往下挤，不会重叠。'),
            new window.Types.ContentBlock(window.Types.BlockType.MARGIN_NOTE, {
                noteText: '这是第二条批注。如果批注太多旁注栏放不下了，溢出的批注会移到下一页的旁注栏顶部，并在当前页旁注栏底部显示"(续下页)"标记。',
                anchoredBlockId: ''
            }),
            p('批注会跟着它锚定的正文走——如果正文因为编辑被挤到下一页，对应的批注也要跟着移到下一页的旁注栏。'),
            new window.Types.ContentBlock(window.Types.BlockType.MARGIN_NOTE, {
                noteText: '第三条批注，用于测试多条批注的自动避让和分页处理功能。',
                anchoredBlockId: ''
            }),
            p('感谢使用本排版工具！如有任何问题或建议，欢迎反馈。本工具演示了现代排版引擎的核心技术原理，包括专业断行、智能分页、图文混排、脚注处理、**自动目录生成**、*图表自动编号*、交叉引用和页边批注等功能，希望能对您的工作有所帮助。')
        ];
